...
export function Event({ event }: Props) {
	...
	return (
		<div
			className={`border-l-2 pl-3 py-2 transition-all duration-300 w-max min-w-[100%] border-gray-600   ${
				isShining ? "bg-amber-800" : "bg-gray-900/50"
			}`}
		>
				...
				<div className="flex items-start">
					{event.type === WebsocketEventC2SEnum.MONSTER_BOUGHT ? (
						<span className="text-green-400">
							&gt; <span className={"text-white"}>{event.userPseudo}</span> a
							acheté <span className="text-blue-400">{event.monsterName}</span>
						</span>
					) : (
						<span className="text-red-400">
							&gt; <span className="text-blue-400">{event.monsterName}</span> de{" "}
							<span className={"text-white"}>{event.userPseudo}</span> s'est
							fait tuer
						</span>
					)}
				</div>
				<span className="pl-2 text-gray-500 text-xs whitespace-nowrap">
					{timeAgo}
				</span>
			</div>
		</div>
